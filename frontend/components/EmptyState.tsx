...
export default function EmptyState() {
  return (
    <div className="bg-[#111820] rounded-xl border border-white/6 p-6 text-center py-16">
      ...
      <h3 className="text-lg font-semibold text-[#e4e8ef] mb-2">
        Ready to evaluate
      </h3>
      <p className="text-sm text-[#8c9aad] mb-8 max-w-md mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-3 max-w-3xl mx-auto">
        {DIMENSIONS.map((d) => (
          <div
            key={d.name}
            className="p-3 rounded-lg bg-[#1a2230] border border-white/6"
          >
            <div className="text-xs font-medium text-[#e4e8ef] leading-tight">
              {d.name}
            </div>
            <div className="text-xs text-[#55667a] mt-0.5 font-mono">
              {d.weight}
            </div>
          </div>
        ))}
